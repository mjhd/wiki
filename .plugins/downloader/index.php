...
<body>
	<h3>All uploads go into /upload folder (file explorer root)</h3>
	<?php
		if((!empty($_POST['action'])) && ($_POST['action']=="download")):
			...
			if(file_exists($download_dir . '/' . $_POST['filesave'])):
				unlink($download_dir . '/' . $_POST['filesave']);
			endif;
			if(!copy($_POST['filedownload'], $download_dir . '/' . $_POST['filesave'])):
				echo("failed to copy file");
			else:
				echo "Copy Completed Successfully<br />";
				echo 'MD5 ' . md5_file($download_dir . '/' . $_POST['filesave']) . "<br />";
			endif;
		else:
	?>
	<form class="download_form" method="post" action="#" name="download_form">
		...
		<ul>
			...
			<li>
				<label for="File to download">URL to download file:</label>
				<input type="text" name="filedownload" placeholder="File Name Here" required/><br />
			</li>
			<li>
				<label for="File to Save">Save as filename:</label>
   ...
   <?php endif; ?>
   <div style="padding: 20px 0 20px 165px;">
   		<button class="submit" type="button" id="file_explorer" style="background-color: #0087ff !important; background: #0087ff !important; border-color: #0087ff !important;">File Explorer /></button>
   </div>
   <script type="text/javascript">
   		document.getElementById('file_explorer').addEventListener('click', function(){ window.location = '../../upload' })
	</script>
</body>
</html>
